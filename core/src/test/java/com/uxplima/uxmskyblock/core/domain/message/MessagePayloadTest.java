...
/**
 * A stored notice carries values, never a sentence.
 *
...
class MessagePayloadTest {
    ...
    void whatWasPackedComesBack() {
        ...
        assertThat(MessagePayload.unpack(MessagePayload.pack(values))).containsExactlyEntriesOf(values);
    }
    ...
    void anAwkwardNameSurvives() {
        ...
        assertThat(MessagePayload.unpack(MessagePayload.pack(values))).isEqualTo(values);
    }

    @Test
    @DisplayName("Nothing packed is nothing unpacked")
    void nothingIsNothing() {
        assertThat(MessagePayload.pack(Map.of())).isEmpty();
        assertThat(MessagePayload.unpack("")).isEmpty();
    }

    @Test
    @DisplayName("A row written before this shape existed still reads back as a body")
    void anOlderRowStillReads() {
        assertThat(MessagePayload.unpack("{\"inviter\":\"Leader\"}"))
    ...
    void aSeparatorCannotForgeAPair() {
        ...
        Map<String, String> read = MessagePayload.unpack(MessagePayload.pack(values));
